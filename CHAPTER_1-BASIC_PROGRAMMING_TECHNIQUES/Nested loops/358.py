...
class FillCirclesApp:
    def __init__(self, circles):
        self.circles = circles
        self.n = len(circles)
        self.has_intersection = find_intersecting_circles(circles)
        self.intersecting_indices = [i for i, v in enumerate(self.has_intersection) if v]

        self.root = tk.Tk()
        self.root.title("Закрашенные круги (пересекающиеся)")

        # Информационная панель
        frame = tk.Frame(self.root)
        frame.pack(side=tk.TOP, fill=tk.X)
        info = f"Всего окружностей: {self.n}, пересекающихся: {len(self.intersecting_indices)}"
    ...
    def compute_transform(self):
        # Определяем область с учетом радиусов только для пересекающихся (но можно для всех, чтобы центрировать)
        # Для корректного масштабирования используем все окружности, чтобы непересекающиеся тоже были видны (но мы их не рисуем)
    ...
    def draw(self):
        ...
        # Рисуем только пересекающиеся окружности, залитые одним цветом
        fill_color = "lightgray"
        outline_color = "black"
        for idx in self.intersecting_indices:
            ...
            # Подпись номера (для ориентира)
            self.canvas.create_text(cx, cy, text=str(idx+1), font=('Arial', 8), fill='black')

        # Если есть непересекающиеся, можно нарисовать их пунктиром для контекста (опционально)
        # Раскомментируйте, если хотите видеть и непересекающиеся:
        # for idx in range(self.n):
        #     if not self.has_intersection[idx]:
        #         x, y, r = self.circles[idx]
        #         cx, cy = self.transform(x, y)
        #         r_scaled = r * self.scale
        #         self.canvas.create_oval(cx - r_scaled, cy - r_scaled,
